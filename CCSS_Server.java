...
public class CCSS_Server {
	...
	public static void main(String[] args) throws IOException {
		new CCSS_Server(6789).run();
	}

	public CCSS_Server(int port) {
		this.port = port;
	...
	public void run() throws IOException {
		...
		System.out.println("Port 6789 is now open.");
...
class ClientHandler implements Runnable {
	...
	private CCSS_Server server;
	private InputStream client;

	public ClientHandler(CCSS_Server server, InputStream client) {
		this.server = server;
		this.client = client;
	}
